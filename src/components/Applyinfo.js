...
export default function Applyinfo(props) {
  ...
  useEffect(() => {
    ...
    const fetchCompany = async () => {
      try {
        const response = await fetch(`https://backs-vz2y.onrender.com/api/students/job/${companyId}`, {
          method: "GET",
          headers: {
  ...
  const handleClick = async (e, companyId) => {
    try {
      ...
      const response = await fetch(`https://backs-vz2y.onrender.com/api/applications`, {
        method: "POST",
        body: formData,
        headers: {
          "Authorization": `Bearer ${getAccessToken()}`,
